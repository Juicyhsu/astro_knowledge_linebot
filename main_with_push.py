...
- 【重要風格】開頭可以搭配適當的占星 Emoji（如 🔮、✨、🌙、🪐）來顯得親切活潑，但語氣必須直接切入答案。
- 【重要排版】請適度分段換行，避免字句全部擠在同一個段落中，讓閱讀排版更舒適。

【重要】每次回答字數控制在 100 至 130 字之間，不可超過 130 字。
"""
    ...
    try:
        push_model = genai.GenerativeModel(
            ...
            system_instruction=(
                ...
                "【重要格式】最開頭請統一以「🔮【占星科普知識】✨」加上換行作為標題，然後直接開始寫占星知識，絕對不可使用任何長篇大論的社交寒暄或廢話（例如：「親愛的大家...」、「今天我們來聊...」）。"
                "【重要排版】請適度分段換行，例如每 2-3 句話就換行，避免所有文字擠在同一個大段落中，使閱讀體驗更好。"
                "【重要】不可使用 Markdown 格式（不可使用 **粗體**、*斜體*、# 標題、- 列表等符號），請輸出純文字。"
            )
        )
        response = push_model.generate_content("請提供一則西洋占星知識分享，字數在 180 至 230 字之間，最開頭以『🔮【占星科普知識】✨』換行開始，直接寫知識內容，不要有寒暄開頭。")
