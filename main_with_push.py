...
def save_latest_push(text):
    try:
        with open(LATEST_PUSH_FILE, "w", encoding="utf-8") as f:
            f.write(text)
    except Exception as e:
        print(f"[Push] Error saving latest push: {e}")
...
- 【重要開頭】直接切入主題回答，不可使用任何長篇大論的「開頭廢話」或多餘社交寒暄（例如：「您好！很高興為您解答...」、「親愛的，關於你問的...」）。
- 【重要排版】請適度分段換行，避免字句全部擠在同一個段落中，讓閱讀排版更舒適。
...
def message_text(event):
    # 保存群組 ID，並建立獨立的 session_key（隔離私訊 vs 群組記憶）
    ...
    try:
        push_model = genai.GenerativeModel(
            ...
            system_instruction=(
                ...
                "使用繁體中文。請控制字數在 180~230 字之間，不可超過 230 字。語氣要親切專業。"
                "【重要格式】最開頭請統一以「🔮占星小知識✨」加上換行作為標題，然後直接開始寫占星知識，絕對不可使用任何長篇大論的社交寒暄或廢話（例如：「親愛的大家...」、「今天我們來聊...」）。"
                "【重要排版】請適度分段換行，例如每 2-3 句話就換行，避免所有文字擠在同一個大段落中，使閱讀體驗更好。"
                "【重要】不可使用 Markdown 格式（不可使用 **粗體**、*斜體*、# 標題、- 列表等符號），請輸出純文字。"
            )
        )
        response = push_model.generate_content("請提供一則西洋占星知識分享，字數在 180 至 230 字之間，最開頭以『🔮占星小知識✨』換行開始，直接寫知識內容，不要有寒暄開頭。")
