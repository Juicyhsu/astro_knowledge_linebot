...
【重要】每次回答字數控制在 150 至 250 字之間，以約 200 字左右最佳，上限不可超過 250 字（亦可彈性縮短至 150 字左右）。
"""
    ...
    try:
        push_model = genai.GenerativeModel(
            ...
            system_instruction=(
                ...
                "使用繁體中文。請控制字數在 250~300 字之間，不可超過 300 字。語氣要親切專業。"
                "【重要】不可使用 Markdown 格式（不可使用 **粗體**、*斜體*、# 標題、- 列表等符號），請輸出純文字。"
            )
        )
        response = push_model.generate_content("請提供一則西洋占星知識分享，字數在 250 至 300 字之間。")
